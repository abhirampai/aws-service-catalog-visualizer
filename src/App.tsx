...
function initialModel(): { source: string; model: NormalizationResult; diagnostics: ParseDiagnostic[] } {
  ...
  return {
    source,
    model: result.document ? normalizeParameters(result.document) : { definitions: [], outputs: [], warnings: [], productName: 'CloudFormation product', productDescription: 'Configure this product' },
    ...
    <main className="app-shell">
      ...
      <div className="workspace">
        ...
        <section className="workspace-panel preview-panel" aria-labelledby="provisioning-preview-heading">
          ...
          <ProvisioningForm definitions={model.definitions} outputs={model.outputs} warnings={model.warnings} productName={model.productName} productDescription={model.productDescription} onReview={() => undefined} />
        </section>
      </div>
    </main>
  )
